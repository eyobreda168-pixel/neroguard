    ...
    <>
      <motion.nav
        ...
        className={cn(
          ...
          isScrolled
            ? "bg-nero-deep/90 backdrop-blur-xl border-b border-border/50 shadow-lg"
            : "bg-transparent"
        )}
      >
            ...
            <div className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <button
                  ...
                  className="relative font-heading text-sm font-medium text-muted-foreground hover:text-foreground transition-colors group"
                >
                  {link.label}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            <div
              className="absolute inset-0 bg-nero-deep/95 backdrop-blur-xl"
              ...
              <div className="flex flex-col gap-4">
                {navLinks.map((link, index) => (
                  <motion.button
                    ...
                    onClick={() => scrollToSection(link.href)}
                    className="text-left font-heading text-lg font-medium text-foreground py-3 border-b border-border/30"
                  >
                    {link.label}
                  </motion.button>
                ))}
